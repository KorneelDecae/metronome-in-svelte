<script lang="ts">
	...
	let beats = writable(4);

	$: metronome.update((s) => ({ ...s, bpm: $bpm }));
	$: metronome.update((s) => ({ ...s, beatsPerMeasure: $beats }));

	let pulse = false;
	$: if ($metronome.beat !== undefined) {
		pulse = true;
		setTimeout(() => (pulse = false), 100);
	}
</script>
...
<main class="app">
	<div class="card">
		...
		<input type="range" min="40" max="240" bind:value={$bpm} class="slider" />

		<div class="time-signature">
			<label for="time-signature">Maat:</label>
			<select id="time-signature" bind:value={$beats}>
